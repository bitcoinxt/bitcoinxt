...
static void multiset_from_gej_var(secp256k1_multiset *target, const secp256k1_gej *input) {
    if (input->infinity) {
        ...
        secp256k1_fe_get_b32(target->data, &input->x);
        secp256k1_fe_get_b32(target->data+32, &input->y);
        secp256k1_fe_get_b32(target->data+64, &input->z);
    }
}
...
static void gej_from_multiset_var(secp256k1_gej *target,  const secp256k1_multiset *input) {
    secp256k1_fe_set_b32(&target->x, input->data);
    secp256k1_fe_set_b32(&target->y, input->data+32);
    secp256k1_fe_set_b32(&target->z, input->data+64);
