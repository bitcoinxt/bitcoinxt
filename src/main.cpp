...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, uint64_t nTime)
{
    ...
    if (txSize > Params().MaxTransactionSize(nTime))
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    if (!CheckTransaction(tx, state, GetAdjustedTime()))
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if ((block.nVersion & SIZE_FORK_VERSION) && 
        ...
        CChainParams::SetSizeForkTime(tAllowBigger);
    }
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown = false)
{
    ...
    if (!fKnown) {
        while (vinfoBlockFile[nFile].nSize + nAddSize >= Params().MaxBlockSize(nTime)*MIN_BLOCKFILE_BLOCKS) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Size limits
    uint64_t nMaxBlockSize = Params().MaxBlockSize(block.GetBlockTime());
    if (block.vtx.empty() ||
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        uint64_t nMaxBlocksize = Params().MaxBlockSize(GetAdjustedTime());
