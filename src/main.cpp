...
// Requires cs_main
bool PeerHasHeader(NodeStatePtr& state, CBlockIndex *pindex)
{
...
bool ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    {
        LOCK(cs_main);
        NodeStatePtr(pfrom->GetId())->fPreferHeaders = true;
    }
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        bool fCanDirectFetch = chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - Params().GetConsensus().nPowTargetSpacing * 20;
        NodeStatePtr nodestate(pfrom->GetId());
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !chainActive.Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        !blocksInFlight.isInFlight(pindexWalk->GetBlockHash())) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                BOOST_REVERSE_FOREACH(CBlockIndex *pindex, vToFetch) {
                    ...
                    MarkBlockAsInFlight()(pfrom->GetId(), pindex->GetBlockHash(), Params().GetConsensus(), pindex);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        //
        {
            ...
            vector<CBlock> vHeaders;
            bool fRevertToInv = (statePtr->fPreferHeaders || pto->vBlockHashesToAnnounce.size() > MAX_BLOCKS_TO_ANNOUNCE);
            ...
            if (!fRevertToInv) {
                ...
                BOOST_FOREACH(const uint256 &hash, pto->vBlockHashesToAnnounce) {
                    ...
                    if (fFoundStartingHeader) {
                        ...
                        vHeaders.push_back(pindex->GetBlockHeader());
                    } else if (PeerHasHeader(statePtr, pindex)) {
                        continue; // keep looking for the first new block
                    } else if (pindex->pprev == NULL || PeerHasHeader(statePtr, pindex->pprev)) {
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    if (!PeerHasHeader(statePtr, pindex)) {
                ...
                statePtr->pindexBestHeaderSent = pBestIndex;
            }
