...
void TxConfirmStats::removeTx(unsigned int entryHeight, unsigned int nBestSeenHeight, unsigned int bucketindex)
{
    ...
    if (nBestSeenHeight == 0)  // the BlockPolicyEstimator hasn't seen any blocks yet
        blocksAgo = 0;
    if (blocksAgo < 0) {
        LogPrint(Log::ESTIMATEFEE, "Blockpolicy error, blocks ago is negative for mempool tx\n");
    ...
    if (blocksAgo >= (int)unconfTxs.size()) {
        if (oldUnconfTxs[bucketindex] > 0) {
            ...
            LogPrint(Log::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from >25 blocks,bucketIndex=%u already\n",
                     bucketindex);
        }
    }
    else {
        ...
        if (unconfTxs[blockIndex][bucketindex] > 0) {
            ...
            LogPrint(Log::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from blockIndex=%u,bucketIndex=%u already\n",
...
void CBlockPolicyEstimator::removeTx(uint256 hash)
{
    ...
    if (pos == mapMemPoolTxs.end()) {
        LogPrint(Log::ESTIMATEFEE, "Blockpolicy error mempool tx %s not found for removeTx\n",
                 hash.ToString().c_str());
        return;
    }
...
void CBlockPolicyEstimator::processBlockTx(unsigned int nBlockHeight, const CTxMemPoolEntry& entry)
{
    ...
    if (blocksToConfirm <= 0) {
        ...
        LogPrint(Log::ESTIMATEFEE, "Blockpolicy error Transaction had negative blocksToConfirm\n");
        return;
    }
