...
void CNode::PushVersion()
{
    ...
    if (IsStealthMode()) {
        ...
        PushMessage("version", 70002, services, nTime, addrYou, addrMe, nLocalHostNonce, FormatSubVersion("Satoshi", CLIENT_VERSION, std::vector<string>(), ""), nBestHeight, true);
    } else {
        PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe, nLocalHostNonce, FormatSubVersion(CLIENT_NAME, CLIENT_VERSION, std::vector<string>(), CLIENT_VERSION_XT_SUBVER), nBestHeight, true);
    }
}
                        ...
                        BOOST_FOREACH (CNode* n, vNodes) {
                            ...
                            if (nodePriority < ipgroup.priority) {
                                LogPrintf("Connection slots exhausted, evicting peer %d with priority %d (group %s) to free up resources\n",
                                          n->id,
                                          nodePriority,
                                          ngroup.name == "" ? string("default") : ngroup.name);
                                n->fDisconnect = true;
                                disconnected = true;
...
CNode::CNode(SOCKET hSocketIn, CAddress addrIn, std::string addrNameIn, bool fInboundIn) : ssSend(SER_NETWORK, INIT_PROTO_VERSION),
