...
static void CheckMinimallyEncoded(std::vector<uint8_t> data,
...
BOOST_AUTO_TEST_CASE(minimize_encoding_test) {
    ...
    for (size_t i = 0; i < MAX_SCRIPT_ELEMENT_SIZE; i++) {
        zero.push_back(0x00);
        CheckMinimallyEncoded(zero, {});

        negZero.push_back(0x80);
        CheckMinimallyEncoded(negZero, {});
    ...
    for (size_t i = 0; i < MAX_SCRIPT_ELEMENT_SIZE; i++) {
        CheckMinimallyEncoded(npadded, n);
        npadded.push_back(0x00);

        CheckMinimallyEncoded(negnpadded, negn);
    ...
    for (size_t i = 0; i < MAX_SCRIPT_ELEMENT_SIZE; i++) {
        CheckMinimallyEncoded(kpadded, k);
        kpadded.push_back(0x00);

        CheckMinimallyEncoded(negkpadded, negk);
