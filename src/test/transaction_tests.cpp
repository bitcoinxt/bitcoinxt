...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            BOOST_CHECK_MESSAGE(CheckTransaction(tx, state, 0), strTest);
...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            CValidationState state;
            fValid = CheckTransaction(tx, state, 0) && state.IsValid();
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    stream >> tx;
    CValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(tx, state, 0) && state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(tx, state, 0) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
