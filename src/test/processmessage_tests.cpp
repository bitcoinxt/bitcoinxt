...
BOOST_AUTO_TEST_CASE(compactblockprocessor_fetch_full) {
    ...
    // create a likely invalid compact block
    // (grow it to be likely too big)
    CompactBlock block(TestBlock1(), CoinbaseOnlyPrefiller{});

    uint64_t currMaxBlockSize = 1000 * 100; // 100kb (for faster unittest)
    size_t tooManyTx = std::ceil(currMaxBlockSize * 2 / minTxSize()) + 1;
    ...
    // should have requested a full block instead.
    BOOST_CHECK_EQUAL(1, node.messages.size());
    BOOST_CHECK_EQUAL("getdata", node.messages.at(0));
}
