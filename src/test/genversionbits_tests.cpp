...
static const unsigned int TEST_BIT = 8;
...
protected:
    int64_t BeginTime(const Consensus::Params& params) const override { return GenVBTestTime(10000); }
    int64_t EndTime(const Consensus::Params& params) const override { return GenVBTestTime(20000); }
    int Period(const Consensus::Params& params) const override { return 1000; }
    int Threshold(const Consensus::Params& params) const override { return 900; }
    int MinLockedBlocks(const Consensus::Params& params) const override { return 1001; }
    int64_t MinLockedTime(const Consensus::Params& params) const override { return 0; }

    // test for fork bit
    bool Condition(const CBlockIndex* pindex, const Consensus::Params& params) const override { return (pindex->nVersion & (1 << TEST_BIT)); }

public:
    ThresholdState GetState(const CBlockIndex* pindexPrev) const {
        return AbstractThresholdConditionChecker::GetStateFor(pindexPrev, Params().GetConsensus(), cache);
    }
};
...
protected:
    ...
    int Period(const Consensus::Params& params) const override { return 1000; }
    int Threshold(const Consensus::Params& params) const override { return 900; }
    int MinLockedBlocks(const Consensus::Params& params) const override { return 0; }
    int64_t MinLockedTime(const Consensus::Params& params) const override { return 5000 * 600; }  // this is a minimum *duration* after lock-in time
...
public:
    ...
    GenVersionBitsTesterMinBlocks& TestDefined() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetState(vpblock.empty() ? NULL : vpblock.back().get()) == THRESHOLD_DEFINED, strprintf("Test %i for DEFINED", num));
            }
        }
        num++;
        return *this;
    ...
    GenVersionBitsTesterMinBlocks& TestStarted() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetState(vpblock.empty() ? NULL : vpblock.back().get()) == THRESHOLD_STARTED, strprintf("Test %i for STARTED", num));
            }
        }
        num++;
        return *this;
    ...
    GenVersionBitsTesterMinBlocks& TestLockedIn() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetState(vpblock.empty() ? NULL : vpblock.back().get()) == THRESHOLD_LOCKED_IN, strprintf("Test %i for LOCKED_IN", num));
            }
        }
        num++;
        return *this;
    ...
    GenVersionBitsTesterMinBlocks& TestActive() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetState(vpblock.empty() ? NULL : vpblock.back().get()) == THRESHOLD_ACTIVE, strprintf("Test %i for ACTIVE", num));
            }
        }
        num++;
        return *this;
    ...
    GenVersionBitsTesterMinBlocks& TestFailed() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetState(vpblock.empty() ? NULL : vpblock.back().get()) == THRESHOLD_FAILED, strprintf("Test %i for FAILED", num));
            }
        }
        num++;
        return *this;
